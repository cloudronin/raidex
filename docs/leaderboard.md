...
do not need any of this. Run `pip install raidex` and see the [README](../README.md).

The board has three moving parts, all in this monorepo:

- [`backend/`](../backend/): the eval **service** that drains the submit queue, calls `raidex.core`, and uploads results to the dataset.
- [`space/`](../space/): the Hugging Face **Space** (Gradio app) with the leaderboard, the capability-vs-RAI gap visual, model cards, and submit form.
- The two Hub datasets, `cloudronin/raidex-results` (scores) and `cloudronin/raidex-requests` (queue), are *generated data* and not in the repo.
...
runs it on a schedule; it requires the provider keys and `HF_TOKEN` as repo secrets.

## Reproducing the published board

The leaderboard was produced by running the roster at Tier A+B. `backend/rerun.py` runs the
rate-limited batch with per-provider throttling and resume; the always-greedy OpenAI/Anthropic
...
calibration. Numbers reproduce within the stated error bars (composite 95% half-width about
±2 points), not bit-for-bit.
...
`cloudronin/raidex-space` Space. It requires the `HF_TOKEN` repo secret.

## See also

- [`space/METHODOLOGY.md`](../space/METHODOLOGY.md): index design, generative-task creation, judging, sampling, normalization, and the shared-lab and sycophancy disclosures.
- [`backend/README.md`](../backend/README.md): service internals and full reproduction detail.
